...
// submit actions
	...
	if ($autonewscount > 0  && $queuecount == '' && $neededfields == 1 && $ready2move == 1) {
		...
		echo '<p>' , PHP_EOL
		   ...
		   , '</p>' , PHP_EOL
		   , '<p><span class="thick larger">This package comes with the new nukeWYSIWYG 3.6.3 Editor!</span></p>' , PHP_EOL
		   , '<p>Source: <a href="http://nukeseo.com/modules.php?name=News&amp;file=article&amp;sid=168" target="_blank">nukeSEO.com</a></p>' , PHP_EOL
		   , 'If you have uploaded the new files, you are required to manually edit config.php. Since this file contains your database information we can not provide a pre-edited file. Please find the following code snippet at the bottom of your config.php file and replace it with next snippet' , PHP_EOL
		   , '<p><strong>find in config.php</strong>:</p>' , PHP_EOL;		   
		...
		$snippet1  = 'if (defined(\'INCLUDE_PATH\') && file_exists(INCLUDE_PATH . \'rnconfig.php\')) {' . PHP_EOL
				   . '	require_once INCLUDE_PATH . \'rnconfig.php\';' . PHP_EOL
				   . '} else {' . PHP_EOL
				   . '	echo <<< ERROR' . PHP_EOL
				   ...
				   . '	<head>' . PHP_EOL
				   . '		<title>Unable To Locate RN CONFIG file!</title>' . PHP_EOL
				   . '	</head>' . PHP_EOL
				   . '	<body>' . PHP_EOL
				   . '		Unable to locate the RavenNuke&trade; configuration file - rnconfig.php<br />' . PHP_EOL
				   . '		<br />' . PHP_EOL
				   . '' . PHP_EOL
				   . '		It could be missing or not readable. Please verify that the file exists and is readable in the root folder.<br />' . PHP_EOL
				   . '		<br />' . PHP_EOL
				   ...
				   . 'ERROR;' . PHP_EOL
				   . '}' . PHP_EOL;
		highlight_num($snippet1);
		echo '<p><strong>replace it in config.php</strong>:</p>' , PHP_EOL;
		$snippet2 = 'require_once dirname(__FILE__) . \'/rnconfig.php\';';
		highlight_num($snippet2);
		echo '<p><strong>Changing CK Skin</strong>:</p>' , PHP_EOL
		   , 'The CK Editor comes with 3 skins. They called kama (default skin), v2 (old look &amp; feel) and Office2003 (MS look). If you want change the skin, you must set a call to a new class inside your theme.php or in your modules. Leave it blank for default theme kama.:<br /><br />' , PHP_EOL;		   
...
function install ($newssort, $newsorder, $newsyearmin, $newsyearmax, $hideautotimes, $previewstory, $hideautosubmit, $archivedefault, $archivetopics, $jqueryselect, $archive_charlimit, $counttopic, $counttitle, $usenotes, $time2, $time3, $slock, $moveautonews, $movesubmitnews) {
	...
	if ($archivetopics == 1) {
		...
		if ($archivetopicsquery) {
			...
			echo 'Hide topics in archive - query failed! ' , $exclamation , '<br />' , PHP_EOL;	
		}
	} else {
	...
	if ($time2 == 1) {
		$time2query = $db->sql_query('ALTER TABLE `' . $prefix . '_stories` ADD `time2` DATETIME NOT NULL DEFAULT 0');
		if ($time2query) {
	...
	if ($time3 == 1) {
		$time3query = $db->sql_query('ALTER TABLE `' . $prefix . '_stories` ADD `time3` DATETIME NOT NULL DEFAULT 0');
		if ($time3query) {
