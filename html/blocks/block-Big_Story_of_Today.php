...
$sql = 'SELECT `sid`, `title` FROM `' . $prefix . '_stories` WHERE (' . ($newssort == 1 ? '`time2`' : '`time`') . ' LIKE \'%' . $tdate . '%\') ' . $querylang . ' AND `slock` = 0 ORDER BY `counter` ' . $newsordertype . ' LIMIT 0,1';
$query = $db->sql_query($sql);
$numrows = $db->sql_numrows($query);
if ($numrows == 0 AND $multilingual == 1) {
	$sql = 'SELECT `sid`, `title` FROM `' . $prefix . '_stories` WHERE (' . ($newssort == 1 ? '`time2`' : '`time`') . ' LIKE \'%' . $tdate . '%\') AND `slock` = 0 ORDER BY `counter` ' . $newsordertype . ' LIMIT 0,1';
	$query = $db->sql_query($sql);
}
