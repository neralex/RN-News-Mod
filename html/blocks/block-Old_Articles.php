...
if ($multilingual == 1) {
    if ($categories == 1) {
        $querylang = 'WHERE `catid` = \'' . intval($cat) . '\' AND (`alanguage` = \'' . $currentlang . '\' OR `alanguage` = \'\') AND `slock` = 0 ' . $newssorting;
    } else {
        $querylang = 'WHERE (`alanguage` = \'' . $currentlang . '\' OR `alanguage` = \'\') AND slock=0 ' . $newssorting;
        if ($new_topic != 0) {
            $querylang .= ' AND `topic` = \'' . intval($new_topic) . '\' AND `slock` = 0 ' . $newssorting;
        }
    }
} else {
    if ($categories == 1) {
        $querylang = 'WHERE `catid` = \'' . intval($cat) . '\' AND `slock` = 0 ' . $newssorting;
    } else {
        $querylang = 'WHERE `slock` = 0 ' . $newssorting;
        if ($new_topic != 0) {
            $querylang = 'WHERE `topic` = \'' . intval($new_topic) . '\' AND `slock` = 0 ' . $newssorting;
        }
    }
}
if (!isset($side)) { $side = ''; }
...
$sql = 'SELECT `sid`, `title`, `time`, `comments`, `time2` FROM `' . $prefix . '_stories` ' . $querylang . ' LIMIT ' . $storynum . ', ' . $oldnum;
...
if ($numrows == 0 AND $multilingual == 1) {
	$sql = 'SELECT `sid`, `title`, `time`, `comments`, `time2` FROM `' . $prefix . '_stories` WHERE `slock` = 0 ' . $newssorting . ' LIMIT ' . $storynum . ', ' . $oldnum;
	$result = $db->sql_query($sql);
}
while (list($sid, $title, $time, $comments) = $db->sql_fetchrow($result)) {
    ...
    preg_match('/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})/', ($newssort == 1 ? $time2 : $time), $datetime2);
