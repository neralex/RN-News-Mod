...
function show_month($year, $month, $month_l) {
    ...
    $result = $db->sql_query('SELECT `sid`, `catid`, `title`, `time`, `comments`, `counter`, `topic`, `alanguage`, `score`, `ratings` FROM `'.$prefix.'_stories` WHERE `slock` = 0 AND `time` >= \''.$year.'-'.$month.'-01 00:00:00\' AND `time` <= \''.$year.'-'.$month.'-31 23:59:59\' ORDER BY `sid` DESC');
...
function show_all($min) {
	...
	echo '<table border="0" width="100%"><tr>'
		...
		, '<td bgcolor="' , $bgcolor2 , '" align="center"><span class="thick">' , _DATE , '</span></td>'
		, '<td bgcolor="' , $bgcolor2 , '" align="center"><span class="thick">' , _ACTIONS , '</span></td></tr>';
	$result = $db->sql_query('SELECT `sid`, `catid`, `title`, `time`, `comments`, `counter`, `topic`, `alanguage`, `score`, `ratings` FROM `' . $prefix . '_stories` WHERE `slock` = 0 ORDER BY `sid` DESC `LIMIT` ' . $min . ',' . $max);
