...
if ($db->sql_numrows($result) > 0) {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		$topicname = htmlspecialchars($row['topicimage'], ENT_QUOTES, _CHARSET);
		$topicimage = htmlspecialchars($row['topicname'], ENT_QUOTES, _CHARSET);
		...
		$sql = 'SELECT `sid`, `catid`, `title`, `informant` FROM `' . $prefix . '_stories` WHERE `topic` = \'' . $topicid . '\' AND `slock` = 0 ORDER BY `sid` DESC LIMIT 0,10';
		...
		if ($num != 0) {
			while ($row2 = $db->sql_fetchrow($result2)) {
				...
				$informant = htmlspecialchars($row2['informant'], ENT_QUOTES, _CHARSET);
				$row3 = $db->sql_fetchrow($db->sql_query('SELECT `title` FROM `' . $prefix . '_stories_cat` WHERE `catid` = \'' . $catid . '\''));
				$rtitle = htmlspecialchars($row3['title'], ENT_QUOTES, _CHARSET);
				if ($catid == 0) {
					$cat_link = '';
				} else {
		...
		if ($num >= 1) {
			$sql2 = 'SELECT `sid`, `catid`, `title`, `hometext`, `time`, `informant` FROM `' . $prefix . '_stories` WHERE `topic` = \'' . $topicid . '\' AND `slock` = 0 ORDER BY `sid` DESC LIMIT 0,1';
			$result2 = $db->sql_query($sql2);
			$row2 = $db->sql_fetchrow($result2);
			$s_sid = $row2['sid'];
			$title = htmlspecialchars($row2['title'], ENT_QUOTES, _CHARSET);
			$informant = htmlspecialchars($row2['informant'], ENT_QUOTES, _CHARSET);
			$hometext = check_html($row2['hometext']);
			$date = $row2['time'];
			echo '<div class="text-center"><p class="thick">' , _TOPICS_MOSTRECENTART , '</p></div>'
			   , '<p><span class="thick"><a href="modules.php?name=News&amp;file=article&amp;sid=' , $s_sid , '">' , $title , '</a></span><br />' , _TOPICS_BY , ' '
