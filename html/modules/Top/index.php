...
if ($multilingual == 1) {
	$queryalang = 'WHERE (`alanguage`="' . $currentlang . '" OR `alanguage`="") AND `slock` = 0'; /* top stories */
	$querya1lang = 'WHERE (`alanguage`="' . $currentlang . '" OR `alanguage`="") AND `slock` = 0 AND'; /* top stories */
	...
	$queryalang = 'WHERE `slock` = 0';
	$querya1lang = 'WHERE `slock` = 0 AND';
	$queryslang = '';
	$queryplang = '';
	$queryrlang = '';
}
...
if ($db->sql_numrows($result) > 0) {
	...
	while (list($uname, $counter) = $db->sql_fetchrow($result, SQL_NUM)) {
		if ($counter > 0) {
			echo '<strong><span class="larger">&middot;</span></strong>&nbsp;' , $lugar , ': <a href="modules.php?name=Your_Account&amp;op=userinfo&amp;username=' , urlencode($uname) , '">' , htmlspecialchars($uname, ENT_QUOTES, _CHARSET) , '</a> - (' , $counter , ' ' , _NEWSSENT , ')<br />';
			$lugar++;
		}
	}
	echo '</td></tr></table><br />';
...
if ($db->sql_numrows($result) > 0) {
	...
	for ($count = 0; $count < count($resultArray); $count++) {
		...
		$pollTitle = htmlspecialchars($pollTitle, ENT_QUOTES, _CHARSET);
...
if ($db->sql_numrows($result) > 0) {
	...
	while (list($aid, $counter) = $db->sql_fetchrow($result, SQL_NUM)) {
		$aid = htmlspecialchars($aid, ENT_QUOTES, _CHARSET);
		if ($counter>0) {
			echo '<strong><span class="larger">&middot;</span></strong>&nbsp;' , $lugar , ': <a href="modules.php?name=Search&amp;query=&amp;author=' , urlencode($aid) , '">' , $aid , '</a> - (' , $counter , ' ' , _NEWSPUBLISHED , ')<br />';
			$lugar++;
		}
	}
	echo '</td></tr></table><br />';
