...
if ($type == 'users') {
	...
	list($st_title) = $db->sql_fetchrow($db->sql_query('SELECT `title` FROM `' . $prefix . '_stories` WHERE `slock`=0 AND `sid`=\'' . $sid . '\''));
	if (!empty($sid)) $instory = ' AND sid=\'' . $sid . '\'';
	$search_type = _SEARCHINSTORY . ' ' . htmlspecialchars($st_title, ENT_QUOTES, _CHARSET);
} else {
	$search_type = _SEARCHIN . ' ' . htmlspecialchars($topictext, ENT_QUOTES, _CHARSET);
}
...
while($row = $db->sql_fetchrow($result)) {
	...
	$content .= '<option' . $sel . ' value="' . $row['topicid'] . '">' . htmlspecialchars($row['topictext'], ENT_QUOTES, _CHARSET) . '</option>';
}
...
while ($row = $db->sql_fetchrow($result)) {
	...
	$content .= '<option' . $sel . ' value="' . $row['catid'] . '">' .  htmlspecialchars($row['title'], ENT_QUOTES, _CHARSET) . '</option>';
}
...
while($row = $db->sql_fetchrow($result)) {
	...
	$content .= '<option' . $sel . ' value="' . $row['aid'] . '">' . htmlspecialchars($row['aid'], ENT_QUOTES, _CHARSET) . '</option>';
}
...
if ($type == 'stories' || !$type) {
	...
	$sql = 'SELECT s.`sid`, s.`aid`, s.`informant`, s.`title`, s.`time`, s.`hometext`, s.`bodytext`, a.`url`, s.`comments`, s.`topic` FROM `' . $prefix . '_stories` s, `' . $prefix . '_authors` a '
	     . 'WHERE s.slock=0 AND s.aid=a.aid ' . $queryalang . ' ' . $categ;
	if (isset($query)) {
	...
	if (!empty($query)) {
		...
		if ($nrows > 0) {
			...
			while($row = $db->sql_fetchrow($result) and $x < $offset) {
				...
				$informant = (!empty($row['informant'])) ? '<a href="modules.php?name=Your_Account&amp;op=userinfo&amp;username=' . urlencode($row['informant']) . '">' . htmlspecialchars($row['informant'], ENT_QUOTES, _CHARSET) . '</a>' : $anonymous;
				$aaid = htmlspecialchars($row['aid'], ENT_QUOTES, _CHARSET);
				$url = (!empty($row['url'])) ? '<a href="' . $row['url'] . '">' . $aaid . '</a>' : $aaid;
				...
				echo '<tr><td><img src="images/folders.gif" alt="" />&nbsp;<span class="option thick"><a href="' , $furl , '">' , htmlspecialchars($row['title'], ENT_QUOTES, _CHARSET) , '</a></span><br />'
	...
	if (!empty($query)) {
		...
		if ($nrows > 0) {
			...
			while($row = $db->sql_fetchrow($result) and $x < $offset) {
				$row_res = $db->sql_fetchrow($db->sql_query('SELECT `title` FROM `' . $prefix . '_stories` WHERE `slock` = 0 and `sid` = \'' . $row['sid'] . '\''));
				$reply = $db->sql_numrows($db->sql_query('SELECT * FROM `' . $prefix . '_comments c`, `' . $prefix . '_storiess` c WHERE s.`slock` = 0 and c.`pid` = \'' . $row['tid'] . '\''));
				$furl = 'modules.php?name=News&amp;file=article&amp;sid=' . $row['sid'] . '#' . $row['tid'];
				$name = (!empty($row['name'])) ? '<a href="modules.php?name=Your_Account&amp;op=userinfo&amp;username=' . urlencode($row['name']) . '">' .  htmlspecialchars($row['name'], ENT_QUOTES, _CHARSET) . '</a>' : $anonymous;
				$datetime = formatTimestamp($row['date']);
				echo '<tr><td><img src="images/folders.gif" alt="" />&nbsp;<span class="option thick"><a href="' , $furl , '">' , htmlspecialchars($row['subject'], ENT_QUOTES, _CHARSET) , '</a></span>'
				   , '<div class="content">' , _POSTEDBY , ' ' , htmlspecialchars($row['name'], ENT_QUOTES, _CHARSET) , ' ' , _ON , ' ' , $datetime , '<br />' , _ATTACHART , ': ' , htmlspecialchars($row_res['title'], ENT_QUOTES, _CHARSET) , '<br />';
				if ($reply == 1) {
					echo '(' , $reply , ' ' , _SREPLY , ')';
					if (is_admin($admin)) {
	...
	if (!empty($query)) {
		...
		if ($nrows > 0) {
			while($row = $db->sql_fetchrow($result) and $x < $offset) {
				...
				echo '<tr><td><img src="images/folders.gif" alt="" />&nbsp;<span class="option thick"><a href="' , $furl , '">' , htmlspecialchars($row['title'], ENT_QUOTES, _CHARSET) , '</a></span>'
	...
	if (!empty($query)) {
		...
		if ($nrows > 0) {
			while($row = $db->sql_fetchrow($result) and $x < $offset) {
				...
				echo '<tr><td><img src="images/folders.gif" alt="" />&nbsp;<span class="option thick"><a href="' , $furl , '">' , htmlspecialchars($row['username'], ENT_QUOTES, _CHARSET) , '</a></span>'
