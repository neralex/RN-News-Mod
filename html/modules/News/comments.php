...
elseif (isset($_GET['sid'])) {
	...
	$sid = '';
	Header('Location: modules.php?name=' . $module_name); // neralex: added for not existing sid
}
if (isset($_POST['tid'])) {
...
switch ($op) {
	...
	default:
		if (!empty($tid) AND ($pid==0)) {
			...
			if (!isset($pid)) $pid = 0;
			DisplayTopic($sid, $pid, $tid, $mode, $order, $thold);
				//echo '</td></tr></table>' . PHP_EOL;  // addresses compliance issue
				echo '</div>' , PHP_EOL;  // neralex: div vs table fix > addresses compliance issue
			if ((($anonpost == 0 AND is_user($user)) OR $anonpost == 1) AND $acomm == 0) {
				...
				wysiwyg_textarea('comment', '', 'PHPNuke', '100%', '100px');
				...
				echo '<input type="hidden" name="pid" value="',$pid,'" />'
					...
					,'<input type="submit" style="cursor:pointer" name="op" value="',_OK,'" />'
...
function navbar($sid, $title, $thold, $mode, $order) {
	...
	$un_title = htmlspecialchars($un_title, ENT_QUOTES, _CHARSET);
	if (!isset($thold)) {
		$thold = 0;
	}
	...
	if ($anonpost == 1 OR (isset($admin) AND is_admin($admin)) OR is_user($user)) {
		...
		echo '<form action="modules.php?name=' , $module_name , '&amp;file=comments" method="post">'
			...
			, '<input type="submit" style="cursor:pointer" value="' , _REPLYMAIN , '" /></form></div></td></tr>';
	}
...
function reply($pid, $sid, $mode, $order, $thold) {
	...
	if ($anonpost == 0 AND !is_user($user)) {
		...
		wysiwyg_textarea('comment', '', 'NukeUser', '100%', '300px');
