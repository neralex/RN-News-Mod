...
function tagList() {
	...
	echo '<div align="justify" class="box">';
		$result = $db->sql_query('SELECT tg.`tag` FROM `' . $prefix . '_tags` tg, `' . $prefix . '_stories` s WHERE tg.`cid` = s.`sid` AND s.`slock` = 0 GROUP BY tg.`tag` ORDER BY RAND()');
		while ($row = $db->sql_fetchrow($result)) {
			$tag = addslashes(check_words(check_html($row['tag'], 'nohtml')));
			$num = $db->sql_numrows($db->sql_query('SELECT tg.`tag` FROM `' . $prefix . '_tags` tg WHERE tg.`tag` = \'' . $tag . '\''));
			if ($num<=1) { $dim = 'class1'; }
...
function getList($tag) {
	...
	//News
	if ($result = $db->sql_query('SELECT * FROM `' . $prefix . '_stories` AS `nw` JOIN `' . $prefix . '_tags` AS `tg` ON nw.`sid` = tg.`cid` WHERE tg.`whr` = 3 AND tg.`tag` = \'' . $tag . '\' AND nw.`slock` = 0 ORDER BY nw.`sid` DESC LIMIT 50')) {
