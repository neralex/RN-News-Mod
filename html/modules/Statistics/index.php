...
function Stats_Main() {
    ...
    $snum = $db->sql_numrows($db->sql_query('SELECT `sid` FROM `' . $prefix . '_stories` WHERE `slock` = 0'));
    $cnum = $db->sql_numrows($db->sql_query('SELECT c.`tid` FROM `' . $prefix . '_comments` c, `' . $prefix . '_stories` s WHERE c.`sid` = s.`sid` AND `slock` = 0'));
    $subnum = $db->sql_numrows($db->sql_query('SELECT `sid` FROM `' . $prefix . '_stories` WHERE `slock` = 1'));
    if (is_active("Topics")) {
