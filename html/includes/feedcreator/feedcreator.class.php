...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "        <item>\n";			
			if (_CHARSET == 'UTF-8' || _CHARSET == 'utf-8') {
 ...
 class AtomCreator10 extends FeedCreator {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "    <entry>\n";
			if (_CHARSET == 'UTF-8' || _CHARSET == 'utf-8') {
				$feed.= "        <title>".htmlspecialchars(htmlspecialchars_decode(utf8_decode(strip_tags($this->items[$i]->title)), ENT_QUOTES), ENT_QUOTES, _CHARSET)."</title>\n";
			} else {
