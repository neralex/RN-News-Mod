...
###############################################################################
	...
	foreach ($bookmarks as $sitename => $sitedetails)
	{
		...
		// neralex: added onclick="socialshare();" and change the target from _blank to popup to open the sharelinks in a popup
		$bookmarkHTML .= '<a href="' . $siteurl . '" title="' . $imgalt . '" target="popup" onclick="socialshare();" class="donkeytopic">';
	...
	// neralex: added to open the sharelinks in a popup
	$socialshare  = '<script type="text/javascript">' . PHP_EOL;
	$socialshare .= '	function socialshare(){ window.open (\'\', \'popup\', \'width=640, height=450, scrollbars=yes, toolbar=no, status=no, resizable=yes, menubar=no, location=no, directories=no, top=310\'); }' . PHP_EOL;
	$socialshare .= '</script>' . PHP_EOL;
	addJSToBody($socialshare,'inline');